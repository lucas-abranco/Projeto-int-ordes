...
export class OrdersService {
 ...
 async addToCart(userId: string, itemData: any) {
  ...
  // --- MUDANÇAS CRÍTICAS NO FLUXO DE PEDIDOS ---
  ...
  async createOrder(userId: string, orderData: any) {
    ...
    const order = this.orderRepo.create({
      ...
      totalPrice: total,
      // CORREÇÃO 1: Status inicial agora é 'OPEN' para aparecer para o motorista
      status: 'OPEN', 
    ...
    // Emite evento, mas o pedido já nasce disponível para teste
  ...
  async getAvailableOrders() {
      return this.orderRepo.find({
          // CORREÇÃO 2: Busca por 'OPEN' para casar com a criação
  ...
  async acceptOrder(driverId: string, orderId: string) {
    await this.orderRepo.update(
      { id: orderId }, 
      // CORREÇÃO 3: Status técnico 'IN_ROUTE'
  ...
  async completeOrder(driverId: string, orderId: string) {
    await this.orderRepo.update(
      { id: orderId }, 
      // CORREÇÃO 4: Status técnico 'DELIVERED'
      { status: 'DELIVERED' } 
    );
